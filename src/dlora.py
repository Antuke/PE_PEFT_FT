            ...
            if has_tasks:
                ...
                if trainable_scale_per_task:
                    self.lora_task_scale = nn.ParameterDict(
                        {
                            task: nn.Parameter(torch.FloatTensor([lora_task_scale]))
                            for task in tasks
                        }
                    )
                else:
                    self.lora_task_scale = {
                        task: lora_task_scale[task] for task in tasks
                    }
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, "lora_tasks_A"):
            for task in self.tasks:
                nn.init.kaiming_uniform_(self.lora_tasks_A[task], a=math.sqrt(5))
    ...
    def forward(self, x: torch.Tensor, x_tasks: Dict[str, torch.Tensor] = None):
        ...
        if self.shared_mode == "matrix":
            lora = (
                ...
                @ self.lora_shared_B.transpose(0, 1)
            ...
            lora_tasks = (
                {
                    ...
                    + lora
                    + (
                        (x if x_tasks is None else x_tasks[task])
                        @ self.lora_tasks_A[task].transpose(0, 1)
                        @ self.lora_tasks_B[task].transpose(0, 1)
                        * self.lora_task_scale[task]
                    )
            ...
            lora = self.lora_norm(
                torch.sum(torch.stack(list(lora_tasks.values()), dim=0), dim=0)
            )
...
class MTLoRAQKV(LoRALayer):
    ...
    def forward(self, x: torch.Tensor, x_tasks: Dict[str, torch.Tensor] = None):
        return (
            torch.cat(
                [self.q(x, x_tasks)[0], self.k(x, x_tasks)[0], self.v(x, x_tasks)[0]],
                dim=-1,
            ),
            {
                task: torch.cat(
                    [
                        self.q(x, x_tasks)[1][task],
                        self.k(x, x_tasks)[1][task],
                        self.v(x, x_tasks)[1][task],
                    ],
                    dim=-1,
                )
                for task in self.tasks
...
def map_old_state_dict_weights(
    state_dict: Dict, mapping: Mapping, prefix: str, split_qkv: bool = False
) -> Dict:
    unmatched_keys = []
    for checkpoint_name, attribute_name in mapping.items():
        ...
        if full_checkpoint_name in state_dict:
            ...
            weights = state_dict.pop(full_checkpoint_name)
            last_four = ".".join(full_attribute_name.split(".")[-4:])
            if split_qkv and last_four in [
                "attn.qkv.linear.weight",
                "attn.qkv.linear.bias",
            ]:
                w_q, w_k, w_v = torch.chunk(weights, chunks=3)
                weight_bias = last_four.split(".")[-1]
                full_attribute_name_without_suffix = ".".join(
                    full_attribute_name.split(".")[:-2]
                )
                state_dict[
                    f"{full_attribute_name_without_suffix}.q.linear.{weight_bias}"
                ] = w_q
                state_dict[
                    f"{full_attribute_name_without_suffix}.k.linear.{weight_bias}"
                ] = w_k
                state_dict[
                    f"{full_attribute_name_without_suffix}.v.linear.{weight_bias}"
                ] = w_v
            else:
